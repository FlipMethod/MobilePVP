...
local PBHUDOptionsFrame = CreateFrame("Frame")

function PBHUDOptionsFrame:OnEvent(event, addOnName)
	if addOnName == "Pet Battle Hud" then
		self:InitializeOptions()
	end
end

function PBHUDOptionsFrame:InitializeOptions()
	self.panel = CreateFrame("Frame")
	self.panel.name = "Pet Battle HUD"

	-- local f_Title = self.panel:CreateFontString(nil, "OVERLAY", "GameFontNormal", 1)
	-- f_Title:SetPoint("TOPLEFT", 20, 10)
	-- f_Title:SetTextColor(0, 1, 1, 1)
	-- f_Title:SetFontString("PET BATTLE HUD") 
	-- GetAddOnMetadata("pet-battle-hud", "Title").." v:"..GetAddOnMetadata("pet-battle-hud", "Version"))
	
	local MissingPets = CreateFrame("CheckButton", nil, self.panel, "InterfaceOptionsCheckButtonTemplate")
	if PBHUD_Profile.b_show_missing == nil then PBHUD_Profile.b_show_missing = false end
	MissingPets:SetChecked(PBHUD_Profile.b_show_missing)	
	MissingPets:SetPoint("TOPLEFT", 20, -20)
	MissingPets.Text:SetText("Show missing zone pet information")
	MissingPets:HookScript("OnClick", function(_, btn, down)
	    PBHUD_Profile.b_show_missing = MissingPets:GetChecked()
		sml_print(PBHUD_Title,"Show missing pets..."..tostring(PBHUD_Profile.b_show_missing))
	end)

	local OnlyMissingPets = CreateFrame("CheckButton", nil, self.panel, "InterfaceOptionsCheckButtonTemplate")
	if PBHUD_Profile.b_show_only_missing == nil then PBHUD_Profile.b_show_only_missing = false end
	OnlyMissingPets:SetChecked(PBHUD_Profile.b_show_only_missing)	
	OnlyMissingPets:SetPoint("TOPLEFT", 25, -40)
	OnlyMissingPets.Text:SetText("Show missing zone pet information only if there are pets you don't have")
	OnlyMissingPets:HookScript("OnClick", function(_, btn, down)
	    PBHUD_Profile.b_show_only_missing = OnlyMissingPets:GetChecked()
		sml_print(PBHUD_Title,"Show only if missing pets..."..tostring(PBHUD_Profile.b_show_only_missing))
	end)
	
	local PBHPartyHide = CreateFrame("CheckButton", nil, self.panel, "InterfaceOptionsCheckButtonTemplate")
	if PBHUD_Profile.b_party_hide == nil then PBHUD_Profile.b_party_hide = false end
	PBHPartyHide:SetChecked(PBHUD_Profile.b_party_hide)	
	PBHPartyHide:SetPoint("TOPLEFT", 20, -80)
	PBHPartyHide.Text:SetText("Hide while in party or raid")
	PBHPartyHide:HookScript("OnClick", function(_, btn, down)
	    PBHUD_Profile.b_party_hide = PBHPartyHide:GetChecked()
		sml_print(PBHUD_Title,"Hide while in party..."..tostring(PBHUD_Profile.b_party_hide))
	end)
...
function PBHUD_OnUpdate(self)
	if PBHUD_db ~= nil then
		if PBHUDOptionsFrame ~=nil then
			if PBHUDOptionsFrame.MissingPets ~=nil then
				PBHUDOptionsFrame.MissingPets:SetChecked(PBHUD_db.b_show_missing)
			end
			if PBHUDOptionsFrame.OnlyMissingPets ~=nil then
				PBHUDOptionsFrame.OnlyMissingPets:SetChecked(PBHUD_db.b_show_only_missing)
			end
		end
		...
		if PBHUD_db.b_party_hide then
			...
			if ingroup or PBHUD_db.InCombat then
				PBHUD:Hide();
			else
				PBHUD:Show();
			end;
...
function PBHUD_OnLoad(self)
	PBHUD_Title   = GetAddOnMetadata("pet-battle-hud", "Title")
...
function PBHUD_OnEvent(self, event, ...)
	...
	if event == "VARIABLES_LOADED" then
		...
		if PBHUD_db then
			...
			PBHUDOptionsFrame:InitializeOptions()
		end;
	end;
...
function PBHUD_UpdatePetHUD()
	...
	for slot = 1, 3 do
		if pet ~= nil then
			...
			if pet.petID == nil then return end
			...
			local inBattle = C_PetBattles.IsInBattle() -- to see if pet is in battle
			...
			_G["PBHUDPet" .. slot .. "HPTexture"]:SetWidth(2.2 * PBHUD_db.PetTeam[slot].hp_prct);
...
function PBHUDOptions_Toggle()
	InterfaceOptionsFrame_OpenToCategory(PBHUDOptionsFrame.panel)
end;
