...
function S:PetBattleFrame()
	...
	for index, infoBar in pairs(infoBars) do
		...
		if index == 1 then
			...
			infoBar.PetTypeFrame:Point('BOTTOMRIGHT',infoBar.ActualHealthBar.backdrop, 'TOPRIGHT', 0, -60)
			...
			infoBar.PetTypeFrame:Point('BOTTOMLEFT',infoBar.ActualHealthBar.backdrop, 'TOPLEFT', 2, -60)
	...
	hooksecurefunc('PetBattleAuraHolder_Update', function(s)
		...
		for i=1, C_PetBattles_GetNumAuras(s.petOwner, s.petIndex) do
			...
			if (isBuff and s.displayBuffs) or (not isBuff and s.displayDebuffs) then
				...
				frame.Duration:FontTemplate(nil, 32, 'OUTLINE')
				frame.Duration:ClearAllPoints()
				frame.Duration:Point('TOP', frame.Icon, 'BOTTOM', 30, 30)
	...
	if E.private.skins.blizzard.tooltip then
		...
		hooksecurefunc('PetBattleAbilityTooltip_Show', function()
			...
			local point, x, y = 'TOPRIGHT', -4, -260
	...
	f.Ally2:Point('TOPRIGHT', f.Ally2.iconPoint, 'TOPLEFT', 80, -520)
	f.Ally3:Point('TOPRIGHT', f.Ally2, 'TOPLEFT', 50, -75)
	f.Enemy2:Point('TOPLEFT', f.Enemy2.iconPoint, 'TOPRIGHT', -80, -520)
	f.Enemy3:Point('TOPLEFT', f.Enemy2, 'TOPRIGHT', -50, -75)
	...
	bar:Size ((113*6 + 7*10), (112 * 1 + 10*2))
	...
	hooksecurefunc('PetBattleFrame_UpdateActionBarLayout', function()
		...
		bf.SwitchPetButton:Size(115, 115)
