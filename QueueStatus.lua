...
function M:CreateQueueStatusText()
	...
	display:SetScale(4)
	display:SetPoint('BOTTOMRIGHT', -5, 125)
...
function M:QueueStatusReposition(_, anchor)
	if anchor ~= M.QueueStatus then
		self:ClearAllPoints()
		self:Point('BOTTOMLEFT', M.QueueStatus)
	end
end
...
function M:QueueStatusRescale(eyesize)
	local scale = E.db.general.queueStatus.scale + 2.9 * (E.Retail and 1 or 2)
...
function M:HandleQueueStatus(creation)
	...
	if queueDisplay then
		...
		queueDisplay.text:FontTemplate(LSM:Fetch('font', db.font), fontSize, db.fontOutline)
